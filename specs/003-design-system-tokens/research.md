...
- **目視の置き換え(2026-09-22 決定)**: 「ブラウザを開いて確かめる」手順は、可能な範囲を
  Storybook の Interaction Test(`expect` による検証)に置き換える。VRT を入れられない代わりに、
  目視で気づきにくい取り違えをブラウザ上の計算済みスタイル(`getComputedStyle`)で確かめる。
  - `app/AppRoutes.stories.tsx`: フェイクの ApiClient を注入し、ナビゲーションから4画面へ遷移して
    各画面の見出しとデータが表示されること(戻った際にキャッシュから再表示されること)を検証する。
  - `app/theme/ThemeAppearance.stories.tsx`: 書体(Noto Sans JP)・文字サイズ・行高・文字色・
    主ボタンの配色・余白(spacing(2) = 16px)・角丸・影・面の色が、実際の描画でトークンどおりに
    なっていることを検証する。コントラスト比は描画された色から算出するため、CssBaseline や
    コンポーネントの上書きで色が変わっていても検出できる(テーマの値だけを見る単体テストとの違い)。
  - 残る `[手動]` は「のっぺりしていないか」(SC-010)など、値では表せない印象の判断のみとする。
...
- **実測結果(T010、2026-09-19)**: 適用前(MUI 既定テーマ)と適用後の比較。基準値は小数第3位以下を
  切り捨てて `contrast.test.ts` の `BASELINE` に記録し、適用後の値が下回らないことを自動検証している。

  | 組み合わせ | 適用前の色 | 適用前 | 適用後の公式トークン | 適用後 |
  |---|---|---|---|---|
  | 本文 / 背景 | rgba(0,0,0,0.87) / #fff | 16.10 | SolidGray 900 `#1a1a1a` / White | 17.40 |
  | 補足 / 背景 | rgba(0,0,0,0.6) / #fff | 5.74 | SolidGray 700 `#4d4d4d` / White | 8.45 |
  | 主ボタンの文字 / 主色 | #fff / #1976d2 | 4.60 | White / Key 900 `#0017c1` | 11.10 |
  | リンク(主色) / 背景 | #1976d2 / #fff | 4.60 | Key 900 / White | 11.10 |
  | エラー文字 / 背景 | #d32f2f / #fff | 4.97 | Semantic Error 2 `#ce0000` / White | 5.79 |
  | 表の見出し / 面 | rgba(0,0,0,0.87) / #fff | 16.10 | SolidGray 900 / White | 17.40 |

  「アクセシビリティの優先」により選び直したトークンが2つある。いずれも別の公式トークンで基準を
  満たせたため、従来の値(MUI 既定)を維持した箇所は無い。
  - 本文色: 当初想定の SolidGray 800 `#333333` は 12.63 で基準 16.10 を下回るため、900 に変更。
  - エラー色: Semantic Error 1 `#ec0000` は 4.60 で基準 4.97 を下回るため、Error 2 に変更。
