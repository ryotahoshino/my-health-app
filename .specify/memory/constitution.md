...
- Modified principles: なし(原則 I〜IX は文言・番号ともに変更していない)
- Added sections:
  - Core Principle X (デザインシステムに準拠する)
  - 技術スタック に「デザインシステム」「デザイントークン」の項目を追加
  - 非目標 に「デジタル庁デザインシステムのコンポーネント一式の完全移植」を追加
- Removed sections: none
- Numbering note: ソースコード内に「原則N」形式の参照が32箇所あるため、既存原則の番号は
  再割り当てせず、新規原則を X として追記した(後方互換性を保つため)
...
- **コンプライアンスレビュー**: spec・plan・tasks の作成時、および実装完了時に本憲法との
  ...
  X(デザインシステム)は受け入れ条件で明示的に検証する。

**Version**: 1.1.0 | **Ratified**: 2026-08-16 | **Last Amended**: 2026-09-17
